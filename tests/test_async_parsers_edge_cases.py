...
class TestAsyncCSVParserEdgeCases:
    ...
    async def test_csv_parser_unicode_decode_error_with_chardet(self):
        ...
        with patch(
            "text_extractor.async_parsers.async_csv_parser.detect_file_encoding"
        ) as mock_detect:
            mock_detect.return_value = "latin-1"
            with patch(
                "text_extractor.async_parsers.async_csv_parser.HAS_CHARDET", True
            ):
                with patch("pandas.read_csv") as mock_read:
                    mock_read.side_effect = UnicodeDecodeError(
                        "latin-1", b"", 0, 1, "invalid"
                    )
                    with pytest.raises(
                        ValueError,
    ...
    async def test_csv_parser_unicode_decode_error_without_chardet(self):
        ...
        with patch(
            ...
            with patch(
                "text_extractor.async_parsers.async_csv_parser.HAS_CHARDET", False
            ):
                with patch("pandas.read_csv") as mock_read:
                    mock_read.side_effect = UnicodeDecodeError(
                        "utf-8", b"", 0, 1, "invalid"
...
class TestAsyncDOCXParserEdgeCases:
    ...
    async def test_docx_parser_package_not_found_error(self):
        ...
        with patch("docx.Document") as mock_document:
            ...
            with pytest.raises(
                FileNotFoundError, match="DOCX file not found: test.docx"
            ):
        ...
        with patch("pdfminer.high_level.extract_pages") as mock_extract:
            ...
            with patch("pdf2image.convert_from_path") as mock_convert:
                ...
                with patch("pytesseract.image_to_string") as mock_ocr:
                    ...
                    with pytest.raises(
                        RuntimeError, match="Failed to OCR PDF page 1 from test.pdf"
                    ):
