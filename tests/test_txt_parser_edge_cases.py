...
class TestTxtParserEdgeCases:
    ...
    def test_txt_parser_unicode_decode_error_with_chardet(self):
        ...
        with patch('text_extractor.parsers.txt_parser.read_file_with_encoding_detection') as mock_read:
    ...
    def test_txt_parser_unicode_decode_error_without_chardet(self):
        ...
        with patch('text_extractor.parsers.txt_parser.read_file_with_encoding_detection') as mock_read:
            ...
            with patch('text_extractor.parsers.txt_parser.HAS_CHARDET', False):
                with pytest.raises(ValueError, match="Failed to decode text file and 'chardet' is not installed"):
                    parse("test.txt")
    ...
    def test_txt_parser_unicode_decode_error_with_file_path(self):
        ...
        with patch('text_extractor.parsers.txt_parser.read_file_with_encoding_detection') as mock_read:
            ...
            mock_read.return_value = (b"\xff\xfe\x00\x00", "utf-8")
            with patch('text_extractor.parsers.txt_parser.HAS_CHARDET', True):
                with pytest.raises(ValueError, match="Failed to decode text file 'test.txt'"):
                    parse("test.txt")
    ...
    def test_txt_parser_unicode_decode_error_with_encoding_info(self):
        ...
        with patch('text_extractor.parsers.txt_parser.read_file_with_encoding_detection') as mock_read:
            ...
            with patch('text_extractor.parsers.txt_parser.HAS_CHARDET', True):
                ...
                mock_bytes.decode.side_effect = UnicodeDecodeError("latin-1", b"\xff\xfe\x00\x00", 0, 1, "invalid")
                mock_read.return_value = (mock_bytes, "latin-1")
                with pytest.raises(ValueError, match="Failed to decode text file 'test.txt' with encoding 'latin-1'"):
                    parse("test.txt")
    ...
    def test_txt_parser_unicode_decode_error_different_encodings(self):
        ...
        for encoding in test_encodings:
            with patch('text_extractor.parsers.txt_parser.read_file_with_encoding_detection') as mock_read:
                # Create a mock bytes object that raises UnicodeDecodeError when decoded
                mock_bytes = MagicMock()
                mock_bytes.decode.side_effect = UnicodeDecodeError(encoding, b"\xff\xfe\x00\x00", 0, 1, "invalid")
                mock_read.return_value = (mock_bytes, encoding)
                with patch('text_extractor.parsers.txt_parser.HAS_CHARDET', True):
                    with pytest.raises(ValueError, match=f"Failed to decode text file 'test.txt' with encoding '{encoding}'"):
                        parse("test.txt")
    ...
    def test_txt_parser_unicode_decode_error_with_different_file_paths(self):
        ...
        for file_path in test_paths:
            with patch('text_extractor.parsers.txt_parser.read_file_with_encoding_detection') as mock_read:
                ...
                mock_read.return_value = (b"\xff\xfe\x00\x00", "utf-8")
                with patch('text_extractor.parsers.txt_parser.HAS_CHARDET', True):
                    with pytest.raises(ValueError, match=f"Failed to decode text file '{file_path}'"):
                        parse(file_path)
    ...
    def test_txt_parser_unicode_decode_error_with_chardet_detailed_error(self):
        ...
        with patch('text_extractor.parsers.txt_parser.read_file_with_encoding_detection') as mock_read:
            ...
            mock_read.return_value = (b"\xff\xfe\x00\x00", "utf-8")
            with patch('text_extractor.parsers.txt_parser.HAS_CHARDET', True):
                with pytest.raises(ValueError, match="Failed to decode text file 'test.txt' with encoding 'utf-8':"):
                    parse("test.txt")
    ...
    def test_txt_parser_unicode_decode_error_without_chardet_simple_message(self):
        ...
        with patch('text_extractor.parsers.txt_parser.read_file_with_encoding_detection') as mock_read:
            ...
            mock_read.return_value = (b"\xff\xfe\x00\x00", "utf-8")
            with patch('text_extractor.parsers.txt_parser.HAS_CHARDET', False):
                with pytest.raises(ValueError, match="Failed to decode text file and 'chardet' is not installed for encoding detection"):
                    parse("test.txt")
